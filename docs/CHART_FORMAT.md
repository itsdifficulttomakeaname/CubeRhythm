...
**tap / hold / drag / fake_tap / fake_hold / fake_drag:**
```json
...
| `inverse`       | 反比例缓动  | `lambda`（λ），速度函数 `speed(t) = GAME_SPEED / (λt + 1)` |
...
- `inverse`：`∫₀ᵀ GAME_SPEED/(λt+1) dt = GAME_SPEED·ln(λT+1)/λ = d`，解为 `T = (e^(d·λ/GAME_SPEED) − 1) / λ`
